...
GROUP_CASHIER = "yoya_reception_bridge.group_hospital_cashier"
...
# hospital_billing.charge_line.RECEIPT_GROUPS. The standalone Cashier group is
# deliberately ABSENT: hospital_billing hardcodes this tuple and cannot be
# edited from here, so a cashier cannot in fact record a payment yet. See the
# 501 payment endpoint.
RECEIPT_GROUPS = (
    GROUP_RECEPTIONIST,
    GROUP_ACCOUNTANT,
    GROUP_MANAGER,
...
def capability_flags(env):
    ...
    return {
        ...
        # whether the user could take payment in the Odoo UI. A standalone
        # cashier is NOT in RECEIPT_GROUPS, so this is False for them --
        # the required behaviour.
        "record_payment": _in_any(env, RECEIPT_GROUPS),
        # Unambiguous: this API records no payment for anyone yet.
