/**
 * POST /api/radiology/results/[resultId]/validate
 *
 * entered -> validated, through action_validate(). Validation delivers nothing and completes nothing; the image set freezes.
 *
  ...
  try {
    return await forwardOdooResult(
      await postOdooApi<RadSignoffResponse>(
        session.sessionId,
        `${RADIOLOGY_API}/results/${parsed.value}/validate`,
      ),
    );
  } catch (error) {
    return handleRouteError(
      error,
      "radiology_report_validate_failed",
      "Unable to reach the radiology service. The report was not validated.",
    );
  }
}
