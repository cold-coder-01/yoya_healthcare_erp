 ...
 */
const CONTROL =
  "h-8 rounded-md border border-slate-300 bg-white cl-body text-slate-900 outline-none transition-colors focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20";
  ...
  return (
    <section className="flex shrink-0 flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-white px-2 py-1.5 shadow-sm">
      <label className="flex items-center gap-1.5">
        <span className="cl-meta font-bold uppercase tracking-[0.08em] text-slate-500">
          Date
        </span>
        <input
          type="date"
      ...
      <div className="relative flex min-w-[200px] flex-1 items-center">
        ...
        {search ? null : (
          <kbd
            aria-hidden
            className="pointer-events-none absolute right-2 rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 font-mono cl-meta font-semibold leading-none text-slate-400"
          >
            /
          </kbd>
        )}
      ...
      <button
        ...
        onClick={onRefresh}
        disabled={loading}
        className="inline-flex h-8 items-center gap-1.5 rounded-md border border-slate-300 bg-white px-3 cl-secondary font-bold uppercase tracking-wide text-slate-700 outline-none transition-colors hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-emerald-600 disabled:cursor-not-allowed disabled:opacity-50"
      >
        <svg
          aria-hidden
          viewBox="0 0 20 20"
